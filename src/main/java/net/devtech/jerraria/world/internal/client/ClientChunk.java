...
public class ClientChunk extends Chunk {
	...
	public ClientChunk(AbstractWorld world, int chunkX, int chunkY) {super(world, chunkX, chunkY);}
	public ClientChunk(AbstractWorld world, int chunkX, int chunkY, JCTagView tag) {super(world, chunkX, chunkY, tag);}
	...
	void scheduleQuadrantReRender(int quadrantX, int quadrantY, AutoBlockLayerInvalidation reason) {
		...
		if(quadrant == null || quadrant.minInvalidation.ordinal() < reason.ordinal()) {
			int absQuadX = quadrantX * 2, absQuadY = quadrantY * 2;
	...
	static final class BakedClientChunkQuadrant {
		 ...
		 */
		final List<TileBakedClientChunkQuadrantData> opaqueLayers;
		 ...
		 */
		final List<TileBakedClientChunkQuadrantData> sortedTranslucentCommonPrimitiveSets;

		BakedClientChunkQuadrant(
			List<TileBakedClientChunkQuadrantData> layers, List<TileBakedClientChunkQuadrantData> sets) {
			...
			for(TileBakedClientChunkQuadrantData data : Iterables.concat(layers, sets)) {
	...
	record TileBakedClientChunkQuadrantData(AutoBlockLayerInvalidation invalidation, Shader<?> vertexData) {}
