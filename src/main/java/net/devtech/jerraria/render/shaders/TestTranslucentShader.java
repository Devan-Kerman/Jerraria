...
public class TestTranslucentShader extends TranslucentShader<Vec3.F<Color.ARGB<End>>> {
	public static final TestTranslucentShader INSTANCE = createShader(Id.create("jerraria", "translucency_test"),
		TestTranslucentShader::new,
		TestTranslucentShader::new
	);

	protected TestTranslucentShader(
		Id id, VFBuilder<End> builder, Object context) {
		super(id, builder.add(Color.argb("color")).add(Vec3.f("pos")), context, TranslucencyStrategy.SINGLE_PASS_WEIGHTED_BLENDED);
	}

	protected TestTranslucentShader(Shader<Vec3.F<Color.ARGB<End>>> shader, SCopy method) {
		super(shader, method, TranslucencyStrategy.SINGLE_PASS_WEIGHTED_BLENDED);
	}

	public void square(
		Matrix3f mat, float offX, float offY, float width, float height, float z, int color) {
