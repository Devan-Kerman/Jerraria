...
import static net.devtech.jerraria.client.render.glsl.GlslLang.KEYWORDS;
import static net.devtech.jerraria.client.render.glsl.GlslLang.PREPROCESSOR_WITH_CONDITION;
import static java.util.Objects.requireNonNull;
import net.devtech.jerraria.client.render.glsl.ast.*;
import net.devtech.jerraria.client.render.glsl.ast.branching.ForAstNode;
import net.devtech.jerraria.client.render.glsl.ast.branching.IfAstNode;
import net.devtech.jerraria.client.render.glsl.ast.branching.condition.*;
import net.devtech.jerraria.client.render.glsl.ast.declaration.ExternalFieldDeclarationAstNode;
import net.devtech.jerraria.client.render.glsl.ast.declaration.ExternalFieldDeclarationAstNode.ExternalFieldType;
...
import net.devtech.jerraria.client.render.glsl.ast.declaration.LocalVarDeclarationAstNode;
import net.devtech.jerraria.client.render.glsl.ast.declaration.MethodDeclarationAstNode;
import net.devtech.jerraria.client.render.glsl.ast.declaration.StructDeclarationAstNode;
import net.devtech.jerraria.client.render.glsl.ast.declaration.VarDeclarationAstNode;
import net.devtech.jerraria.client.render.glsl.ast.preprocessor.*;
import net.devtech.jerraria.client.render.glsl.ast.util.AstUtils;
import net.devtech.jerraria.client.render.glsl.ast.util.BiPredicate;
import net.devtech.jerraria.client.render.glsl.ast.value.DefineValueAstNode;
import net.devtech.jerraria.client.render.glsl.ast.value.ExtensionStatusValueAstNode;
...
public class GlslParser {
        ...
        do {
            ...
            if (token.getType() == TOKEN_PREPROCESSOR) {
                ...
                if (GlslLang.KW_IF.equals(token.getText())) {
                    ...
                    parseDiscard(token);
                } else if(GlslLang.KW_STRUCT.equals(token.getText())) {
					parseStructDeclaration(token, content);
                ...
                if (level == LEVEL_FILE) {
                    ...
                    if (secondToken.getType() == TOKEN_WORD) {
                        parseVarDeclaration(token, content, LocalVarDeclarationAstNode::new);
                ...
                } else if(level == LEVEL_STRUCT) {
					parseVarDeclaration(token, content, FieldDeclarationAstNode::new);
                }
            }
    ...
    private PreprocessorAstNode parsePreprocessorNode(final Token token, final Token keyWordToken,
	    ...
	    for(int i = keyWordToken.getOffset(); i < content.length; i++) {
		    if(content[i] == '\n') {
			    this.offset = i;
			    break;
		    }
     ...
     * @param content        the content.
     */
    private void parseExternalFieldDeclaration(final Token keywordToken, final char[] content) {
		Token fieldTypeToken;
		if(keywordToken.getText().equals(GlslLang.KW_LAYOUT)) {
			// todo retain layout information
			fieldTypeToken = findToken(content, TOKEN_KEYWORD);
		} else {
			fieldTypeToken = keywordToken;
		}

		final Token typeToken;
		StructDeclarationAstNode struct = null;
        Token nameToken;

	    if(fieldTypeToken.getText().equals("uniform")) {
		    this.saveState();
			typeToken = findToken(content, TOKEN_WORD);
		    nameToken = findToken(content, TOKEN_WORD, TOKEN_LEFT_BRACE);
		    if(nameToken.getType() == TOKEN_LEFT_BRACE) { // ubo declaration
			    this.restoreState();
			    struct = parseStructDeclaration(typeToken, content);
				nameToken = findToken(content, TOKEN_WORD, TOKEN_SEMICOLON);
				if(nameToken.getType() == TOKEN_SEMICOLON) {
					nameToken = null;
				}
		    }
	    } else {
		    typeToken = findToken(content, TOKEN_WORD);
		    nameToken = findToken(content, TOKEN_WORD);
	    }
        ...
        nodeStack.addLast(node);
        try {
            node.setType(struct != null ? struct : parseType(typeToken));
            node.setName(nameToken == null ? null : parseName(nameToken));
	...
	// todo support st'ructs with name right after
	private StructDeclarationAstNode parseStructDeclaration(Token structKeywordToken, char[] content) {
		final Token nameToken = this.findToken(content, TOKEN_WORD);
		final Token startStructToken = findToken(content, TOKEN_LEFT_BRACE);
		final AstNode parent = nodeStack.getLast();
		final StructDeclarationAstNode node = new StructDeclarationAstNode();
		node.setParent(parent);
		node.setLine(structKeywordToken.getLine());
		node.setOffset(structKeywordToken.getOffset());

		nodeStack.addLast(node);
		try {
			node.setNameNode(parseName(nameToken));
			parseSymbol(startStructToken);
			parseContent(content, LEVEL_STRUCT, AstUtils.RIGHT_BRACE);
			final Token endBodyToken = findToken(content, TOKEN_RIGHT_BRACE);
			parseSymbol(endBodyToken);
		} finally {
			nodeStack.removeLast();
